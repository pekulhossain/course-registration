/* eslint-disable react/jsx-key */
/* eslint-disable no-unused-vars */
import { ToastContainer, toast } from 'react-toastify';
import 'react-toastify/dist/ReactToastify.css';
import React, { useEffect } from "react";
import "./Home.css";
import { useState } from "react";
import Cart from "../Cart/Cart";
import photo from "../../assets/giphy.gif";
const Home = () => {
  const [actors, setactors] = useState([]);
  const [selectedActor, setSelectedActor] = useState([]);
  const [totalCost, setTotalCost] = useState(0);
  const [remaining, setRemaining] = useState(0);
  const [control, setControl] = useState(true);
  const Budget = 20;

  useEffect(() => {
    fetch("./data.json")
      .then((res) => res.json())
      .then((data) => setactors(data));
  }, []);

  const handleAddActor = (actor) => {
    const isExist = selectedActor.find((act) => act.id == actor.id);
    let temp = actor?.price;

    if (isExist) {
      return toast("already selected this course");
    } else {
      setSelectedActor([...selectedActor, actor]);
      selectedActor.forEach((item) => {
        temp += item.price;
      });
      const sum = Budget - temp;
      if (sum < 0) {
        return toast("Your Credit is not available");
      } else {
        setTotalCost(temp);
        setRemaining(sum);
      }
    }
  };

  const handleControl = (status) => {
    setControl(status);
  };

  console.log(control);

  return (
    <div className="container">
      {control ? (
        <div className="card-container">
          {actors.map((actor) => (
            <div key={actor.id} className="card">
              <div className="card-img">
                <img className="photo" src={actor?.image} alt="" />
              </div>
              <h2 className="">{actor?.name}</h2>
              <p>
                <small>
                  {actor?.description}
                </small>
              </p>
              <div className="price-container">
                <p> <span className="cardPriceCredit">$ Price: </span> {actor?.credit}</p>
                <p><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
                <path d="M12 6.042C10.3516 4.56336 8.2144 3.74694 6 3.75C4.948 3.75 3.938 3.93 3 4.262V18.512C3.96362 18.172 4.97816 17.9989 6 18C8.305 18 10.408 18.867 12 20.292M12 6.042C13.6483 4.56328 15.7856 3.74685 18 3.75C19.052 3.75 20.062 3.93 21 4.262V18.512C20.0364 18.172 19.0218 17.9989 18 18C15.7856 17.9969 13.6484 18.8134 12 20.292M12 6.042V20.292" stroke="#1C1B1B" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                </svg></p>
                <p> <span className="cardPriceCredit">Credit: </span> {actor?.price}hr</p>

              </div>
              <button
                onClick={() => handleAddActor(actor)}
                className="card-select-btn"
              >
                Select
              </button>
            </div>
          ))}
        </div>
      ) : (
        <div>
          <img src={photo} alt="" />
          <button onClick={() => handleControl(true)}>back</button>
        </div>
      )}
      <div className="cart">
      <ToastContainer />
        <Cart
          selectedActor={selectedActor}
          remaining={remaining}
          totalCost={totalCost}
          handleControl={handleControl}
        ></Cart>
      </div>
    </div>
  );
};

export default Home;
